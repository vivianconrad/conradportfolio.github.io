...
function findHandler(events, callable, delegationSelector = null) {
  return Object.values(events).find(event => event.callable === callable &&
...
const EventHandler = {
  ...
  trigger(element, event, args) {
    ...
    const evt =
        hydrateObj(new Event(event, {bubbles, cancelable : true}), args);
    if (defaultPrevented) {
      evt.preventDefault();
    }
    if (nativeDispatch) {
...
const Manipulator = {
  ...
  removeDataAttribute(
      element,
      key) { element.removeAttribute(`data-bs-${normalizeDataKey(key)}`); },
...
const getSelector = element => {
  ...
  return selector ? selector.split(',').map(sel => parseSelector(sel)).join(',')
                  : null;
};
...
class Carousel extends BaseComponent {
  ...
  _addTouchEventListeners() {
    ...
    const endCallBack = () => {
      ...
      this.touchTimeout =
          setTimeout(() => this._maybeEnableCycle(),
  ...
  _slide(order, element = null) {
    ...
    const nextElement =
        element || getNextActiveElement(this._getItems(), activeElement, isNext,
                                        this._config.wrap);
  ...
  show() {
    ...
    if (this._config.parent) {
      activeChildren = this._getFirstLevelChildren(SELECTOR_ACTIVES)
                           ...
                           .map(element => Collapse.getOrCreateInstance(
    ...
    Object.keys(state.elements).forEach(function(name) {
      ...
      var styleProperties =
          Object.keys(state.styles.hasOwnProperty(name)
                          ? state.styles[name]
                          : initialStyles[name]); // Set all values to an empty
                                                  // string to unset them
      ...
      Object.keys(attributes)
          .forEach(function(attribute) { element.removeAttribute(attribute); });
    });
  };
...
function getUAString() {
  ...
  if (uaData != null && uaData.brands && Array.isArray(uaData.brands)) {
    return uaData.brands
        .map(function(item) { return item.brand + "/" + item.version; })
        .join(' ');
  }
...
function listScrollParents(element, list) {
  ...
  var isBody =
      scrollParent === ((_element$ownerDocumen = element.ownerDocument) == null
                            ? void 0
                            : _element$ownerDocumen.body);
  var win = getWindow(scrollParent);
  var target =
      isBody ? [ win ].concat(win.visualViewport || [],
...
function computeAutoPlacement(state, options) {
  ...
  var placements$1 =
      variation ? flipVariations
                ...
                : basePlacements;
  var allowedPlacements = placements$1.filter(function(
      placement) { return allowedAutoPlacements.indexOf(placement) >= 0; });
  ...
  return Object.keys(overflows).sort(function(
      a, b) { return overflows[a] - overflows[b]; });
}
...
function flip(_ref) {
  ...
  for (var i = 0; i < placements.length; i++) {
    ...
    if (checks.every(function(check) { return check; })) {
...
function preventOverflow(_ref) {
  ...
  if (checkMainAxis) {
    ...
    var maxLen =
        variation === start
            ? -popperRect[len]
            : -referenceRect[len]; // We need to include the arrow in the
                                   // calculation so the arrow doesn't go
    ...
    var clientOffset =
        arrowOffsetParent ? mainAxis === 'y' ? arrowOffsetParent.clientTop || 0
  ...
  if (checkAltAxis) {
    ...
    var _preventedOffset = tether && isOriginSide
                               ...
                               : within(tether ? _tetherMin : _min, _offset,
...
function orderModifiers(modifiers) {
  ...
  return modifierPhases.reduce(function(acc, phase) {
    return acc.concat(orderedModifiers.filter(function(
        modifier) { return modifier.phase === phase; }));
  }, []);
}
...
function popperGenerator(generatorOptions) {
  ...
  return function createPopper(reference, popper, options) {
    ...
    var instance = {
      ...
      setOptions : function setOptions(setOptionsAction) {
        ...
        state.orderedModifiers =
            orderedModifiers.filter(function(m) { return m.enabled; });
        runModifierEffects();
    ...
    function cleanupModifierEffects() {
      effectCleanupFns.forEach(function(fn) { return fn(); });
      effectCleanupFns = [];
    }
...
class Dropdown extends BaseComponent {
  ...
  _getPopperConfig() {
    ...
    if (this._inNavbar || this._config.display === 'static') {
      Manipulator.setDataAttribute(this._menu, 'popper',
                                   'static'); // TODO: v6 remove
...
class FocusTrap extends Config {
  ...
  activate() {
    ...
    EventHandler.off(document,
                     EVENT_KEY$5); // guard against infinite focus loop
...
EventHandler.on(document, EVENT_CLICK_DATA_API$2, SELECTOR_DATA_TOGGLE$2,
                function(event) {
                  ...
                  // avoid conflict when clicking modal toggler while another
                  // one is open
...
EventHandler.on(document, EVENT_CLICK_DATA_API$1, SELECTOR_DATA_TOGGLE$1,
                function(event) {
                  ...
                  // avoid conflict when clicking a toggler of an offcanvas,
  ...
  _setListeners() {
    ...
    for (const trigger of triggers) {
      if (trigger === 'click') {
        ...
        EventHandler.on(
            this._element, eventIn, this._config.selector, event => {
              const context = this._initializeOnDelegatedTarget(event);
        ...
        EventHandler.on(
            this._element, eventOut, this._config.selector, event => {
              const context = this._initializeOnDelegatedTarget(event);
              context
...
class ScrollSpy extends BaseComponent {
  ...
  _activateParents(target) {
    ...
    if (target.classList.contains(CLASS_NAME_DROPDOWN_ITEM)) {
      SelectorEngine
          .findOne(SELECTOR_DROPDOWN_TOGGLE$1,
                   target.closest(SELECTOR_DROPDOWN))
          .classList.add(CLASS_NAME_ACTIVE$1);
      return;
    }
    for (const listGroup of SelectorEngine.parents(target,
